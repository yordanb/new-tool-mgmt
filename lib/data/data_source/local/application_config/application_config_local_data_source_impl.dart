...
class ApplicationConfigLocalDataSourceImpl
  ...
  Future<List<ApplicationConfig>> getAll({
    ...
    List<ApplicationConfig> modelValues = [];
    for (var value in values) {
      modelValues.add(ApplicationConfig.fromJson(value));
    }
    return modelValues;
  }
  ...
  Future<ApplicationConfig?> create({
    ...
    await prefs.setString('application_config', modelValues.toJsonEncoded());
    return newModel;
  }
  ...
  Future<void> deleteAll() async {
    await prefs.setString('application_config', jsonEncode([]));
  }
  ...
  Future<void> createQueue({
    ...
    String jsonList =
        await prefs.getString('application_config_queued_tasks') ?? "[]";
    ...
    await prefs.setString(
        'application_config_queued_tasks', jsonEncode(values));
  }
  ...
  Future<bool> isRunningQueuedTask() async {
    bool isRunning =
        await prefs.getBool('application_config_queued_tasks_running') ?? false;
    return isRunning;
  }

  Future<void> startQueue() async {
    await prefs.setBool('application_config_queued_tasks_running', true);
  }
